...
TARGET_DIR = os.path.join(B, "bernina_bunny")
...
NEG_BASE = (
    ...
    "signature, watermark, tail, demon tail, animal tail"
)
...
CHAR_BASE = (
    "stell4berninabnuy, 1girl, pink hair, gradient hair, short hair with long locks, "
    "red hair ribbon, inverted cross, pink eyes, curled horns, low wings, (large breasts:1.3), "
    "collarbone, navel, hip_bones"
)

OUTFIT_BASE = (
    "fake animal ears, rabbit ears, detached collar, red necktie, necktie between breasts, "
    "black leotard, strapless, highleg leotard, side-tie leotard, half gloves, black gloves, "
    "fishnet pantyhose, thigh belt, red high heels, tight clothes"
)
...
STAGES_7 = [
    ...
    },
    {
        ...
        "clothes": "clothes_lift, torn_leotard, broken_strap, exposed_breasts",
...
def build_single_pose_workflow(pose_name):
    ...
    nodes["lora_char"] = {"class_type": "LoraLoader", "inputs": {"model": ["ckpt", 0], "clip": ["clip_skip", 0], "lora_name": "Stella-Bernina-v1.safetensors", "strength_model": 1.0, "strength_clip": 1.0}}
    ...
    for s in STAGES_7:
        ...
        nodes[f"s_{sn}"] = {"class_type": "SaveImage", "inputs": {"filename_prefix": f"bernina_bunny_{pose_name}_{sn}", "images": [f"d_{sn}", 0]}}
    return nodes
...
def build_master_workflow():
    ...
    for pose_name, (pose_tags, width, height) in POSES.items():
        for s in STAGES_7:
            ...
            nodes[f"s_{key}"] = {"class_type": "SaveImage", "inputs": {"filename_prefix": f"bernina_bunny_{pose_name}_{s['n']}", "images": [f"d_{key}", 0]}}
    return nodes
...
def generate():
    ...
    total_imgs = count * 7
    print(f"\n[DONE] bernina_bunny actualizado a 7 ETAPAS: 17 workflows individuales + workflow_master.json ({total_imgs} imagenes en total).")

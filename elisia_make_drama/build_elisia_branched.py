...
CHAR = "elisia_(make_drama), elisia, make drama, 1girl, demon girl, demon horns, curved horns, black horns, pointy ears, long hair, wavy hair, bangs, delicate face, curvy, hourglass figure, huge breasts, massive cleavage, narrow waist, wide hips, huge ass, big ass, bubble butt, thick thighs"
...
def build_workflow():
    ...
    for pose_name, pose_tags in POSES.items():
        ...
        nodes[f"cn_prep_{pose_name}"] = {
            "class_type": "MiDaS-DepthMapPreprocessor",
            "inputs": {
                "image": [f"d_{pose_name}_01_miedo", 0],
                "a": 6.283185307179586,
                "bg_threshold": 0.1
            }
        }
        
        ref_ksampler = f"k_{pose_name}_01_miedo"
        denoise_map = {
            "01_miedo": 1.0,
            "01.5_resistencia": 1.0,
            "02_dolor": 1.0,
            "03_quebranto": 1.0,
            "04_ahegao": 1.0,
            "04.5_derrame": 1.0,
        ...
        for n, depth, expr, motion, fluids, sound in STAGES:
            ...
            if n != "01_miedo":
                nodes[f"cn_apply_{key}"] = {
                    ...
                    "inputs": {
                        ...
                        "image": [f"cn_prep_{pose_name}", 0]
                    }
                }
            
            if n == "01_miedo":
                latent_input = ["latent_shared", 0]
                positive_input = [f"p_{key}", 0]
            else:
                latent_input = [ref_ksampler, 0]
                positive_input = [f"cn_apply_{key}", 0]
                
            denoise_val = denoise_map[n]
            ...
            nodes[f"s_{key}"] = {
                ...
                "inputs": {
                    "filename_prefix": f"elisia_{pose_name}_{n}",
                    "images": [f"d_{key}", 0]
                }
            }
...
if __name__ == "__main__":
    wf = build_workflow()
    wf_path = os.path.join(B, "workflow_elisia_branched.json")

...
md_content = """# 📚 LoRA Characters Vault — Guía Maestra de Personajes y Entrenamiento
...
3. [Parámetros de Entrenamiento Estándar (Illustrious SDXL)](#-3-parámetros-de-entrenamiento-estándar)
...
## ⚙️ 3. Parámetros de Entrenamiento Estándar (Illustrious SDXL)

Para entrenar con **Kohya_ss (`sdxl_train_network.py`)** en Google Colab:
...
| **Network Dim (Rank)** | `32` | Capacidad suficiente para aprender anatomía sin sobreajuste |
| **Network Alpha** | `16` | Relación estándar 1/2 con el Dim |
| **Resolution** | `1024,1024` (`--enable_bucket`) | Mantiene la máxima resolución de detalle |
| **Learning Rate** | `1e-4` (U-Net) / `5e-5` (Text Encoder) | Tasa de aprendizaje óptima para SDXL |
| **LR Scheduler** | `cosine` con 50 warmup steps | Decaimiento suave para evitar artefactos |
| **Optimizer** | `AdamW8bit` | Ahorro de VRAM para GPUs Colab T4 / L4 |
| **Epochs / Repeats** | `10 epochs`, `10 repeats` por imagen | ~500 a 1000 pasos totales |
| **Precision** | `fp16` o `bf16` | Precisión mixta rápida |
...
p1 = r"C:\Users\NEO\Downloads\LoRA_Characters_Vault\LORA_CHARACTERS_VAULT_GUIDE.md"
p2 = r"E:\ComfyUI\characters\LORA_CHARACTERS_VAULT_GUIDE.md"

for p in [p1, p2]:
    with open(p, "w", encoding="utf-8") as f:
        f.write(md_content.strip())
    print("[OK] Created MD file at:", p)

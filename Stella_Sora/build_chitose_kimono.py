...
TARGET_DIR = os.path.join(B, "chitose_kimono")
...
CHAR_BASE = (
    "dvykmchitose, 1girl, purple eyes, bright pupils, white pupils, multicolored hair, "
    "colored inner hair, black hair, purple hair, twintails, huge breasts, collarbone, navel, hip_bones"
)

OUTFIT_BASE = (
    "japanese clothes, white kimono, bare shoulders, wide sleeves, long sleeves, "
    "puffy sleeves, ribbon trimmed sleeves, off shoulder, pom pom hair ornament, "
    "blue hairclip, x hair ornament, detached collar, blue collar, obi, purple sash, "
    "hakama skirt, pleated skirt, miniskirt, blue skirt, blue hakama, white thigh strap, "
    "thighhighs, white thighhighs"
)
...
STAGES_7 = [
    {
        ...
        "clothes": "intact_clothing, kimono_lift, hakama_lift, panties_aside",
    ...
    },
    {
        ...
        "clothes": "kimono_lift, torn_kimono, exposed_breasts, single_bare_shoulder",
...
def build_single_pose_workflow(pose_name):
    ...
    nodes["lora_char"] = {"class_type": "LoraLoader", "inputs": {"model": ["ckpt", 0], "clip": ["clip_skip", 0], "lora_name": "Chitose_Dovellys.safetensors", "strength_model": 1.0, "strength_clip": 1.0}}
    ...
    for s in STAGES_7:
        ...
        nodes[f"s_{sn}"] = {"class_type": "SaveImage", "inputs": {"filename_prefix": f"Stella_Sora/chitose_kimono/{pose_name}/chitose_kimono_{pose_name}_{sn}", "images": [f"d_{sn}", 0]}}
    return nodes
...
def build_master_workflow():
    ...
    for pose_name, (pose_tags, width, height) in POSES.items():
        for s in STAGES_7:
            ...
            nodes[f"s_{key}"] = {"class_type": "SaveImage", "inputs": {"filename_prefix": f"Stella_Sora/chitose_kimono/{pose_name}/chitose_kimono_{pose_name}_{s['n']}", "images": [f"d_{key}", 0]}}
    return nodes
...
def generate():
    ...
    for pose_name in POSES:
        ...
        count += 1
        print(f"[OK] [{count}/17] {fname} ({POSES[pose_name][1]}x{POSES[pose_name][2]}) -> 7 etapas")
    ...
    total_imgs = count * 7
    print(f"\n[DONE] chitose_kimono completado: 17 workflows individuales + workflow_master.json ({total_imgs} imagenes en total).")

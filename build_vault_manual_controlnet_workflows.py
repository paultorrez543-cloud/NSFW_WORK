...
# ==============================================================================
# 🏛️ MASTER BUILDER: WORKFLOWS CON CONTROLNET MANUAL (LoadImage)
# Permite arrastrar cualquier imagen/pose de referencia para guiar las 5 etapas.
# ==============================================================================
...
def generate_manual_controlnet_workflow(char_key, char_cfg):
    nodes = {}
    node_id = 1
    ...
    nodes[str(node_id)] = {
        ...
        "_meta": {"title": f"LoRA 1: {char_cfg['name'].capitalize()} (Character)"}
    }
    ...
    nodes[str(node_id)] = {
        ...
        "_meta": {"title": "LoRA 2: BBC Interracial (Concept)"}
    }
    ...
    # 5. LoadImage (Pose de Referencia MANUAL)
    nodes[str(node_id)] = {
    ...
    # 6. MiDaS Depth Preprocessor sobre la imagen manual cargada
    nodes[str(node_id)] = {
    ...
    # Generar las 5 Etapas Secuenciales usando la Pose Manual como Guía
    for s_idx, stg in enumerate(STAGES, 1):
        outfit = get_outfit_tags(char_cfg, stg["undress_type"])
        interracial_tags = stg["interracial_tags"]
        stg_prompt = clean_tags(f"score_9, score_8_up, score_7_up, source_anime, rating_explicit, {char_tags}, {outfit}, {interracial_tags}, {atmos_tags}, {stg['slider_tags']}, {stg['expr']}, {stg['action']}, masterpiece, best quality, highly detailed")
        
        # Stage Slider LoRA (Pussy Adjuster)
        nodes[str(node_id)] = {
            "class_type": "LoraLoader",
            "inputs": {
        ...
        # Apply ControlNet Depth usando el mapa extraído de la imagen manual
        nodes[str(node_id)] = {
            ...
            "_meta": {"title": f"Apply Manual Depth ControlNet ({stg['id']})"}
        }
...
def main():
    ...
    print("Generating Manual ControlNet Workflows (LoadImage node) for all characters...")
    ...
    for char_key, char_cfg in CHARACTERS.items():
        ...
        # 1. Generar JSON de Workflow Manual
        wf_nodes = generate_manual_controlnet_workflow(char_key, char_cfg)
        json_path = os.path.join(char_dir, f"workflow_{char_name}_manual_controlnet.json")
        with open(json_path, "w", encoding="utf-8") as f:
            json.dump(wf_nodes, f, indent=2)
        print(f"  [OK] Created Manual ControlNet JSON: {json_path}")
        
        # 2. Generar Script Individual
        script_path = os.path.join(char_dir, f"build_{char_name}_manual_controlnet.py")
        script_content = f'''import os
import json
import sys

# Dedicated builder for {char_name.capitalize()} Manual ControlNet workflow
...
def build():
    ...
    wf = generate_manual_controlnet_workflow("{char_key}", char_cfg)
    out_path = os.path.join(os.path.dirname(__file__), "workflow_{char_name}_manual_controlnet.json")
...
'''
        ...
        print(f"  [OK] Created Script: {script_path}")

    print("\nAll Manual ControlNet workflows for all 10 characters generated successfully!")

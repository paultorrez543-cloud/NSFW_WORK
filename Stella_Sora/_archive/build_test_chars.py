...
CHARS = {
    ...
    "shia": {
        "lora": "Shia_Stella_Sora__シア_ステラソラ.safetensors", "seed": 42424251,
        "char": "shia, stella sora, 1girl, (large breasts:1.3)",
        "outfit": "bunny girl, bunny ears, sailor collar, top, bikini, covered midriff",
        "extra_neg": ", tail, demon tail",
    },
    "bernina_default": {
        "lora": "Stella-Bernina-v1.safetensors", "seed": 42424253,
        ...
        "outfit": "maid headdress, white shrug, puffy sleeves, wide sleeves, black dress, red dress, two-tone dress, hobble dress, long dress, layered dress, strapless, red o-ring, buttons, see-through cleavage, fishnet top, black gloves, fishnet gloves, black bow, high heels boots, black boots",
        "extra_neg": ", tail, demon tail, lantern, holding lantern",
    },
    "bernina_bunny": {
        "lora": "Stella-Bernina-v1.safetensors", "seed": 42424254,
        ...
        "outfit": "fake animal ears, rabbit ears, detached collar, red necktie, necktie between breasts, black leotard, strapless, highleg leotard, side-tie leotard, half gloves, black gloves, fishnet pantyhose, thigh belt, red high heels, tight clothes",
        "extra_neg": ", tail, demon tail",
    },
    "reisen": {
        "lora": "Reisen_ridge_-_Stella_Sora.safetensors", "seed": 42424255,
        ...
        "outfit": "mask on head, plague doctor mask, goggles, goggles on head, jacket, black jacket, open jacket, bowtie, black bowtie, white shirt, underbust, brown belt, shorts, black shorts, white thighhighs, skindentation, black boots, fold-over boots",
        "extra_neg": "",
    },
}
...
for cname, cfg in CHARS.items():
    ...
    nodes[ng] = {"class_type":"CLIPTextEncode","inputs":{"text":NEG + cfg["extra_neg"],"clip":[lc,1]}}

    prompt = f"score_9, score_8_up, source_anime, rating_safe, {cfg['char']}, {cfg['outfit']}, standing, looking at viewer, full body, front_view, simple background, smile, anime, masterpiece, best_quality"
    nodes[f"p_{cname}"] = {"class_type":"CLIPTextEncode","inputs":{"text":prompt,"clip":[lc,1]}}
    nodes[f"k_{cname}"] = {"class_type":"KSampler","inputs":{"seed":cfg["seed"],"steps":20,"cfg":3.5,"sampler_name":"dpmpp_2m","scheduler":"karras","denoise":1,"model":[lc,0],"positive":[f"p_{cname}",0],"negative":[ng,0],"latent_image":["latent_shared",0]}}
    nodes[f"d_{cname}"] = {"class_type":"VAEDecode","inputs":{"samples":[f"k_{cname}",0],"vae":["ckpt",2]}}
    nodes[f"s_{cname}"] = {"class_type":"SaveImage","inputs":{"filename_prefix":f"test_{cname}","images":[f"d_{cname}",0]}}

with open(os.path.join(B, "workflow_test_chars.json"), "w", encoding="utf-8") as f:
    json.dump(nodes, f, indent=2, ensure_ascii=False)

n = sum(1 for v in nodes.values() if v.get("class_type")=="SaveImage")
print(f"✅ workflow_test_chars.json → {n} imagenes (1 por personaje)")
print(f"   standing, simple background, smile | sin sexo | para validar LoRAs")

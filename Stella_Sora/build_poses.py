...
NORMAL_DIR = os.path.join(B, "normal")
...
OUTFITS = {
    ...
    "bunny": {
        "char": (
            "stell4virigiabnuy, 1girl, white hair, long hair, half up braid, blunt bangs, "
            "double-parted bangs, red eyes, demon horns, low wings, mole on hip, large breasts, "
            "collarbone, navel, hip_bones"
        ),
        "base_clothes": (
            "fake animal ears, rabbit ears, lace hairband, black hairband, detached collar, "
            "red bowtie, lace collar, fur armlet, cleavage, white leotard, strapless leotard, "
            "side-tie leotard, cross-laced leotard, highleg leotard, crotch zipper, showgirl skirt, "
            "white thighhighs, single fishnet thighhigh, mismatched legwear, frilled thigh strap, "
            "heart o-ring, wrist cuffs, lace-trimmed wrist cuffs, ankle strap, red high heels"
        ),
        "tail_neg": ", tail, demon tail, animal tail",
    },
}

# (pose_tags, width, height)
POSES = {
    # 25 Poses (1024x1024)
...
STAGES_7 = [
    {
        ...
        "clothes": "intact_clothing, clothes_lift, panties_aside",
    ...
    },
    {
        ...
        "clothes": "clothes_lift, torn_dress, broken_strap, exposed_breasts",
    ...
    },
    {
        ...
        "clothes": "shredded_clothes, single_bare_shoulder, exposed_breasts",
...
def build_workflow(outfit_name, pose_name, prefix_slug):
    ...
    for s in STAGES_7:
        ...
        nodes[f"k_{sn}"] = {"class_type": "KSampler", "inputs": {"seed": SEED, "steps": 40, "cfg": 6.0, "sampler_name": "euler_ancestral", "scheduler": "karras", "denoise": 1, "model": ["lora_size", 0], "positive": [f"p_{sn}", 0], "negative": ["neg", 0], "latent_image": [f"e_{sn}", 0]}}
...
def build_master():
    ...
    for outfit_name, cfg in OUTFITS.items():
        for pose_name, (pose_tags, width, height) in POSES.items():
            for s in STAGES_7:
                ...
                prompt = clean_tag_list(", ".join([p for p in parts if p]))
                key = f"{outfit_name}_{pose_name}_{s['n']}"
                ...
                nodes[f"k_{key}"] = {"class_type": "KSampler", "inputs": {"seed": SEED, "steps": 40, "cfg": 6.0, "sampler_name": "euler_ancestral", "scheduler": "karras", "denoise": 1, "model": ["lora_size", 0], "positive": [f"p_{key}", 0], "negative": ["neg", 0], "latent_image": [f"e_{key}", 0]}}
...
def generate_all():
    ...
    for outfit in OUTFITS:
        for pose in POSES:
            if outfit == "bunny" and pose == "cowgirl":
                fname = "workflow_sequence_bunny.json"
            elif outfit == "default" and pose == "cowgirl":
                fname = "workflow_sequence_default.json"
            elif outfit == "default" and pose == "doggystyle":
                fname = "workflow_sequence_doggystyle.json"
            else:
                fname = f"workflow_sequence_{outfit}_{pose}.json"
            
            slug = f"{outfit}_{pose}"
            wf = build_workflow(outfit, pose, slug)
            
            target_path = os.path.join(NORMAL_DIR, fname)
            with open(target_path, "w", encoding="utf-8") as f:
                json.dump(wf, f, indent=2, ensure_ascii=False)
            total += 1

    master_wf = build_master()
    with open(os.path.join(NORMAL_DIR, "workflow_master.json"), "w", encoding="utf-8") as f:
        json.dump(master_wf, f, indent=2, ensure_ascii=False)

    print(f"\n[DONE] Completado: {total} workflows individuales + workflow_master.json en {NORMAL_DIR} actualizados.")

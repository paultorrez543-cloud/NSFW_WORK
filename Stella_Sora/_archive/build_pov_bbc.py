...
# Progresión de nude: vestida → semidesnuda → desnuda → nude + cum
CHRONOLOGY = [
    # ── Acto 1: Intro (VESTIDA) ──
    ...
    # ── Acto 2: Semidesnuda / Foreplay (BRA + PANTIES) ──
    ("04_pecho",       "pov, bra, skirt, clothes_lift, breast_touching, looking_at_viewer, moaning"),
    ("05_dedos",       "pov, bra, panties, fingering, wet, looking_at_viewer, aroused"),
    ("06_oral",        "pov, bra, panties, blowjob, looking_up_at_viewer, oral, huge_penis, deepthroat"),
    ("07_paizuri",     "pov, nude_top, panties, titfuck, paizuri, black_penis, between_breasts, looking_at_viewer"),

    # ── Acto 3: Desnuda / Inserción (NUDE) ──
    ...
    ("10_estirando",   "pov, nude, stretching, huge_penis, screaming, looking_at_viewer, tip_in_pussy"),
    ("11_ritmo",       "pov, nude, thrusting, moaning, looking_at_viewer, (deep penetration:1.3), motion_lines"),

    # ── Acto 4: Posiciones (NUDE) ──
    ...
    ("18_anal",        "pov, nude, anal_sex, anal_penetration, huge_penis, looking_back_at_viewer"),
    ...
    # ── Acto 5: Creampie Focus (NUDE + CUM) ──
...
for n, tags in CHRONOLOGY:
    ...
    nodes[f"s_{n}"] = {"class_type":"SaveImage","inputs":{"filename_prefix":f"virigia_pov_bbc_{n}","images":[f"d_{n}",0]}}

with open(os.path.join(B, "workflow_virigia_pov_bbc.json"), "w", encoding="utf-8") as f:
    json.dump(nodes, f, indent=2, ensure_ascii=False)

n = sum(1 for v in nodes.values() if v.get("class_type")=="SaveImage")
print(f"✅ workflow_virigia_pov_bbc.json → {n} imagenes (POV + Creampie + BBC + nude progression)")

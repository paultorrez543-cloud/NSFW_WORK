...
def hands_for(stage):
    n = stage["n"]
    if n in ["01_miedo","02_resistencia"]: return "hands_on_floor"
...
for cname, cfg in CHARS.items():
    ...
    for s in STAGES:
        parts = ["score_9, score_8_up, source_anime, rating_explicit", cfg["char"], cfg["outfit"], MALE, DP, s["depth"], POSE, hands_for(s), s["expr"], s["fluids"]]
        ...
        nodes[f"s_{key}"] = {"class_type":"SaveImage","inputs":{"filename_prefix":f"revcowgirl_{cname}_{s['n']}","images":[f"d_{key}",0]}}

with open(os.path.join(B, "workflow_reverse_cowgirl_all.json"), "w", encoding="utf-8") as f:
    json.dump(nodes, f, indent=2, ensure_ascii=False)

n = sum(1 for v in nodes.values() if v.get("class_type")=="SaveImage")
print(f"✅ workflow_reverse_cowgirl_all.json → {n} imagenes")
print(f"   {len(CHARS)} personajes x reverse cowgirl x 10 etapas")

...
# ==============================================================================
# 🏛️ MASTER BUILDER: OPENPOSE WORKFLOWS (controlnet-openpose-sdxl.safetensors)
# - Especializado en esqueletos de postura (huesos, articulaciones y extremidades)
# - Carga: controlnet-openpose-sdxl.safetensors
# - Preprocesador: OpenposePreprocessor (cuerpo, manos, rostro) a resolución 1024
# - Vista previa: Muestra el esqueleto OpenPose en pantalla
# - Raíz Única: 1 solo cálculo de OpenPose distribuido a las 5 etapas
...
def generate_openpose_workflow(char_key, char_cfg):
    nodes = {}
    node_id = 1
    ...
    # 5. ControlNet OpenPose Loader (controlnet-openpose-sdxl.safetensors)
    nodes[str(node_id)] = {
        "class_type": "ControlNetLoader",
        "inputs": {"control_net_name": "controlnet-openpose-sdxl.safetensors"},
        "_meta": {"title": "ControlNet OpenPose Loader (SDXL)"}
    }
    ...
    nodes[str(node_id)] = {
        ...
        "inputs": {"image": "reference_openpose_image.png", "upload": "image"},
        "_meta": {"title": "📥 Cargar Imagen de Pose (Referencia OpenPose)"}
    }
    manual_load_id = node_id
    node_id += 1
    
    # 7. OpenPose Preprocessor (Cuerpo, Manos, Rostro a 1024)
    nodes[str(node_id)] = {
        ...
        "_meta": {"title": "🦴 OpenPose Preprocessor (Esqueleto 1024)"}
    }
    openpose_prep_id = node_id
    node_id += 1
    
    # 7B. Vista Previa del Esqueleto OpenPose
    nodes[str(node_id)] = {
        "class_type": "PreviewImage",
        "inputs": {
            "images": [str(openpose_prep_id), 0]
        },
        "_meta": {"title": "👁️ Esqueleto OpenPose Detectado"}
    }
    node_id += 1
    ...
    for s_idx, stg in enumerate(STAGES, 1):
        ...
        # ControlNetApplyAdvanced (OpenPose SDXL)
        nodes[str(node_id)] = {
            ...
            "inputs": {
                ...
                "image": [str(openpose_prep_id), 0],
                "strength": 0.60 if s_idx == 1 else 0.55,
                "start_percent": 0.0,
                "end_percent": 0.75
            },
            "_meta": {"title": f"Apply OpenPose Advanced (0.0-0.75) [{stg['id']}]"}
        }
        ...
        nodes[str(node_id)] = {
            ...
            "inputs": {
                ...
                "filename_prefix": f"{char_cfg['name']}/openpose/{stg['id']}"
            },
...
def main():
    ...
    print("Generating Specialized OpenPose SDXL Workflows for all 10 characters...")
    ...
    for char_key, char_cfg in CHARACTERS.items():
        ...
        wf_nodes = generate_openpose_workflow(char_key, char_cfg)
        json_path = os.path.join(char_dir, f"workflow_{char_name}_manual_openpose.json")
        with open(json_path, "w", encoding="utf-8") as f:
            json.dump(wf_nodes, f, indent=2)
        print(f"  [OK] Created OpenPose JSON: {json_path}")

    print("\nAll OpenPose SDXL workflows generated successfully!")
